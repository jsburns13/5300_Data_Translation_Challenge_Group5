...
df_Employed <- emp_data %>% drop_na(Industry) %>% group_by(date, Industry) %>% summarise(Count = sum(Employed))
df_unemployed_experienced <- emp_data %>% drop_na(Industry) %>% group_by(date, Industry) %>% summarise(Count = sum(Unemployed_experienced_worker))
df_unemployed_new <- emp_data %>%  group_by(date, Industry) %>% summarise(Count = sum(Unemployed_new_worker))

#Check the education attainments of employed and umemployed workers
df_Employed_EDUC <- emp_data %>% filter(Education != 'NA') %>% group_by(date, Education) %>% summarise(Count = sum(Employed))
df_unemployed_experienced_EDUC <- emp_data %>% filter(Education != 'NA') %>% group_by(date, Education) %>% summarise(Count = sum(Unemployed_experienced_worker))
df_unemployed_new_EDUC <- emp_data %>% filter(Education != 'NA') %>% group_by(date, Education) %>% summarise(Count = sum(Unemployed_new_worker))
