...
emp_data$RACE <- as.character(df$RACE) #Convert RACE into character type
emp_data$AGE <- as.numeric(emp_data$AGE)#Convert AGE into numeric type
emp_data$SEX <- as.character(df$SEX) #Convert SEX into character type
emp_data$MARST <- as.character(df$MARST) #Convert MARST into character type
...
#Filter out whoever is not in the laborforce
df <- df %>% filter(LABFORCE == 2) 
```
...
df_Employed <- emp_data %>% drop_na(Industry) %>% group_by(yearmo, Industry) %>% summarise(Count = sum(Employed))
...
df_Employed_EDUC <- emp_data %>% filter(Education != 'NA') %>% group_by(yearmo, Education) %>% summarise(Count = sum(Employed))
...
df_Employed_WorkerClass <- emp_data %>% filter(ClassofWorker != 'NA') %>% group_by(yearmo, ClassofWorker) %>% summarise(Count = sum(Employed))
...
df_Employed_Workstatus <- emp_data %>% group_by(yearmo, Workstatus) %>% summarise(Count = sum(Employed))
...
df_Employed_Age <- emp_data %>% group_by(yearmo, Age_broad) %>% summarise(Count = sum(Employed))
...
df_Employed_Sex <- emp_data %>% group_by(yearmo, SEX) %>% summarise(Count = sum(Employed))
...
df_Employed_Race <- emp_data %>% group_by(yearmo, RACE) %>% summarise(Count = sum(Employed))
...
df_Employed_Marital <- emp_data %>% filter(MARST != 9) %>% group_by(yearmo, MARST) %>% summarise(Count = sum(Employed))
...
df_Employed_Occupation <- emp_data %>% filter(Occupation != 'NA') %>% group_by(yearmo, Occupation) %>% summarise(Count = sum(Employed))
...
First, a final variable, `Employment`, was added. which classifies EMPSTAT in (10, 12) as `employed` and EMPSTAT in (21,22) as `Unemployed`, and changed pre post to a bool value. Covid status was also changed to a `bool` value. 
```{r}

df$post_covid <- (df$COVIDSTATUS == 'post-covid')
```
...
```{r}
ggplot(df, aes(x= RACE, color = factor(Employment))) + geom_bar() + 
...
ggplot(df, aes(x= indname, color = factor(Employment))) + geom_bar() + 
...
```{r}
ggplot(df, aes(x= Age_broad, color = factor(Employment))) + geom_bar() +
...
```{r}
ggplot(df, aes(x= SEX, color = factor(Employment))) + geom_bar() +
...
```{r}
ggplot(df, aes(x= MARST, color = factor(Employment))) + geom_bar() +
...
```{r}
lpm3 <- feols(Employment ~ post_covid + Education, data = df, se='hetero')
lpm4 <- feols(Employment ~  post_covid + Education + post_covid:Education, data = df, se='hetero')
etable(lpm3,lpm4)
```
...
- post_covidTRUE = -0.0438 indicating that the over all effect of covid had a -4.38 percentage point impact on employment.
...
### _RACE_
```{r}
lpm10 <- feols(Employment ~ post_covid + RACE + post_covid:RACE, data = df, se='hetero')
etable(lpm10)
```
...
### _Matital Status_
```{r}
lpm11 <- feols(Employment ~ post_covid + MARST, data = df, se='hetero')
lpm12 <- feols(Employment ~ MARST + post_covid + post_covid:MARST, data = df, se='hetero')
etable(lpm11,lpm12)
```
